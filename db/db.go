...
func (d *datastoreWrapper) GetLessons(ctx context.Context, cursorStr string, filter Filter, size int) ([]data.Entry, string, error) {
	lessons := make([]data.Entry, 0)
	query := datastore.NewQuery("Entry").Order("-Scraped").Limit(size)
	switch filter {
